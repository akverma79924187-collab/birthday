...
export default function Navigation() {
  ...
  return (
    <>
      ...
      <header
        className={`fixed top-4 left-0 right-0 z-40 px-4 md:px-8 transition-all duration-500 ${
          scrolled ? 'translate-y-0' : 'translate-y-1'
        }`}
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between glass-panel rounded-full py-2.5 px-6 border border-[#e6ca85]/20 shadow-2xl">
          {/* Logo / Name */}
          <Link href="/" className="flex items-center gap-2 group">
            <span className="w-8 h-8 rounded-full bg-gradient-to-tr from-[#e6ca85]/30 to-[#df95a6]/30 flex items-center justify-center border border-[#e6ca85]/40 text-[#e6ca85] group-hover:scale-105 transition-transform">
              <Heart className="w-4 h-4 text-[#e6ca85] fill-[#e6ca85]/30" />
            </span>
            <span className="font-serif-display text-lg font-bold tracking-wider text-gradient text-gold-gradient">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={togglePlay}
              className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#161324] border border-[#e6ca85]/30 text-stone-200 hover:text-[#e6ca85] hover:border-[#e6ca85] transition-all text-xs"
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-full bg-[#161324] border border-white/10 text-stone-200 hover:text-[#e6ca85]"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 lg:hidden bg-black/80 backdrop-blur-xl flex flex-col justify-between p-6"
          >
            <div className="flex items-center justify-between pt-4 pb-6 border-b border-white/10">
              <div className="flex items-center gap-3">
                <Sparkles className="w-5 h-5 text-[#e6ca85]" />
                <span className="font-serif-display text-xl font-bold text-gold-gradient">
            ...
            <div className="grid grid-cols-2 gap-3 py-6 my-auto max-h-[65vh] overflow-y-auto">
              {NAV_ITEMS.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[10px] font-mono text-stone-500">0{idx + 1}</span>
                  </Link>
                );
              })}
            </div>
